...
class HomeScreen extends Component {
  ...
  renderItem = (cv) => {
    return (
      <CvContainer
        nameSurname={
          cv.item.answers[4].answer.first + ' ' + cv.item.answers[4].answer.last
        }
...
function mapDispatchToProps(dispatch) {
  return {
    actions: {
      getCvs: bindActionCreators(cvActions.getCvs, dispatch),
      changeCv: bindActionCreators(cvActions.changeCv, dispatch),
      deleteCv: bindActionCreators(cvActions.deleteCv, dispatch),
    },
  };
}
